...
- `operationally_feasible`: historical output-field name retained for compatibility. In the released datasets it means `feasible`, voltage-feasible under 0.90 to 1.05 p.u., and no failed load-supply check. Thermal feasibility was not evaluated because traceable branch-level thermal ratings were unavailable.
